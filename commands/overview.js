  ...
  RCSHub.registerCommand({
    ...
    render: function () {
      return (
        ...
        '<p style="margin:0;font-size:11px;color:rgba(255,255,255,.45);">Host: ' +
        location.hostname +
        " • " +
        ...
        '<div style="font-size:16px;font-weight:600;">' +
        (RCSHub.modals ? RCSHub.modals.size : 0) +
        "</div>" +
        "</div>" +
        '<div style="background:rgba(255,255,255,0.015);border:1px solid rgba(255,255,255,0.03);border-radius:10px;padding:6px 8px;">' +
        ...
        "</div>" +
        "</div>" +
        '<div style="font-size:11px;color:rgba(255,255,255,0.65);">Quer nova função? Cria em <code>src/commands/</code> e coloca no <code>manifest.json</code>.</div>' +
        "</div>"
      );
    },
  });
